...
two_stage_test <- function(mask_list, allele_freq_test, two_stage_threshold = 3,
  ...
  while(!to_return & i <= num_masks){
    ...
    test_stat_var <- sum(2*sample_size*allele_freq_test_temp$AF*(1-allele_freq_test_temp$AF))/residual_variance
    test_stat <- test_stat_num/sqrt(test_stat_var)
    pval <-  2*pnorm(-abs(test_stat)) #Looks like I get the right answer comparing to RAREMETAL output but do a more comprehensive look later
