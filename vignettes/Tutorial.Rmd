...
## Performing Rare-Variant Aggregation Testing Using Annotation File
...
Note that we should be able to also supply the actual VCF used to generate the 
...
agg_test(score_stat_file, vcf_file, chr, burden, wburden,
                     ...
                     pLOF_broadMissense = pLOF_broadMissense,
                     altGroupFilePath = NULL, altCovariancePath = NULL,
                     altRaremetalPath = "trueCovariance_", mafThreshold = 0.01, 
...
```{r}
pLOF_res <- fread("trueCovariance_pLOF.meta.burden.results", header = TRUE, data.table = FALSE)
print(pLOF_res)
```


```{r}
pLOF_narrowMissense_res <- fread("trueCovariance_pLOF_narrowMissense.meta.burden.results", header = TRUE, data.table = FALSE)
print(pLOF_narrowMissense_res)
```


```{r}
pLOF_broadMissense_res <- fread("trueCovariance_pLOF_broadMissense.meta.burden.results", header = TRUE, data.table = FALSE)
print(pLOF_broadMissense_res)
```
